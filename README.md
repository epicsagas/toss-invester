...
> 한국 주식(KRX 코스피·코스닥) 투자 멀티 에이전트 분석·매매 플러그인 —
> TradingAgents 스타일 8단계 디비트 파이프라인을 AI 코딩 에이전트 플러그인 형태로
> 이식. 데이터·주문은 [토스증권 Open API](https://developers.tossinvest.com/docs).
...
**키 발급**: [토스증권 Open API](https://developers.tossinvest.com/)에서 클라이언트 자격증명(`client_id`/`client_secret`) 발급 후 `.env`에 설정. **시세 조회를 포함한 모든 API 호출에 키가 필요하다** (Upbit와 달리 keyless 공개 엔드포인트 없음).
...
전 명령 목록: `python3 scripts/toss.py --help`. 표준 라이브러리만 사용 —
pip 설치 불필요.
